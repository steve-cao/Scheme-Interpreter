...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        return scheme_apply( self, (operands.map(lambda operand: scheme_eval(operand, env))), env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("incorrect number of paramaters")
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        result = complete_eval(scheme_apply(self, operands, env))
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if expressions.second == nil:
            return scheme_eval(expressions.first, env, True)
        if scheme_truep(scheme_eval(expressions.first, env)): #problem
...
def make_let_frame(bindings, env):
    ...
    return env.make_child_frame(params, values)
    # END PROBLEM 15
...
class MuProcedure(UserDefinedProcedure):
    ...
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
        the_frame = env.make_child_frame(self.formals, args)
        return the_frame
    # END PROBLEM 16
